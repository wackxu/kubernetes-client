...
public class PriorityClassListFluentImpl<A extends PriorityClassListFluent<A>> extends io.fabric8.kubernetes.api.builder.BaseFluent<A> implements PriorityClassListFluent<A>{

    private String apiVersion;
    private List<PriorityClassBuilder> items;
    private String kind;
    ...
    public A addToItems(int index,PriorityClass item){
            if (this.items == null) {this.items = new ArrayList<PriorityClassBuilder>();}
...
@Deprecated public List<PriorityClass> getItems(){
    ...
    public PriorityClass buildMatchingItem(io.fabric8.kubernetes.api.builder.Predicate<PriorityClassBuilder> predicate){
            for (PriorityClassBuilder item: items) { if(predicate.apply(item)){return item.build();} } return null;
    }

    public A withItems(List<PriorityClass> items){
            if (this.items != null) { _visitables.removeAll(this.items);}
            if (items != null) {this.items = new ArrayList<PriorityClassBuilder>(); for (PriorityClass item : items){this.addToItems(item);}} else { this.items = new ArrayList<PriorityClassBuilder>();} return (A) this;
    }

    public A withItems(PriorityClass... items){
            if (this.items != null) {this.items.clear();}
            if (items != null) {for (PriorityClass item :items){ this.addToItems(item);}} return (A) this;
    }
...
@Deprecated public ListMeta getMetadata(){
    ...
    public boolean equals(Object o){
            ...
            PriorityClassListFluentImpl that = (PriorityClassListFluentImpl) o;
    ...
    public class ItemsNestedImpl<N> extends PriorityClassFluentImpl<PriorityClassListFluent.ItemsNested<N>> implements PriorityClassListFluent.ItemsNested<N>,io.fabric8.kubernetes.api.builder.Nested<N>{

            private final PriorityClassBuilder builder;
        private final int index;
    
            ItemsNestedImpl(int index,PriorityClass item){
                    this.index = index;
    ...
    public N and(){
            return (N) PriorityClassListFluentImpl.this.setToItems(index, builder.build());
    }
    public N endItem(){
            return and();
    }
    ...
    public class MetadataNestedImpl<N> extends ListMetaFluentImpl<PriorityClassListFluent.MetadataNested<N>> implements PriorityClassListFluent.MetadataNested<N>,io.fabric8.kubernetes.api.builder.Nested<N>{
    ...
    public N and(){
            return (N) PriorityClassListFluentImpl.this.withMetadata(builder.build());
    }
